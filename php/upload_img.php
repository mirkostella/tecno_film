    ...
    class gestione_img{
        ...
        public function uploadImg($directory, $input_name){
            ...
            if($_FILES[$input_name]["size"] > 4000000){
                $error = $error."<div class=\"msg_error_box\">Il file è troppo grande, carica un file di dimensione minore di 4MB.</div>";
                $upload_ok = false;
            }

            //check formato file
            if($imgFileType != "jpg"  && $imgFileType != "png" && $imgFileType!= "jpeg"){
                $error = $error."<div class=\"msg_error_box\">Il file deve avere una delle seguenti estensioni: JPG, PNG, JPEG.</div>";
                $upload_ok = false;
            }
            ...
            if($upload_ok){
                ...
                if(!move_uploaded_file($_FILES[$input_name]["tmp_name"], $uploaded_file)){
                    $error="<div class=\"msg_error_box\">C'è stato un errore nel caricamento del file.</div>";
                }
            }
